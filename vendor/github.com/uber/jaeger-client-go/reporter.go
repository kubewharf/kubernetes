...
func NewRemoteReporter(sender Transport, opts ...ReporterOption) Reporter {
	...
	if options.periodicFlushSupport {
		return NewPeriodicFlushReporter(newTraversalReportPolicy(), reporter)
	}
	return reporter
}
...
const (
	defaultTraversePeriod = 5 * time.Second
)

type reportPolicy interface {
	add(span *Span)
	remove(span *Span)
	reportQueue() <-chan []*Span
}

type traversalReportPolicy struct {
	queue          chan []*Span
	lastReportTime map[*Span]time.Time
	lock           sync.Mutex
	traversePeriod time.Duration
}

func newTraversalReportPolicy() *traversalReportPolicy {
	p := &traversalReportPolicy{
		queue:          make(chan []*Span, defaultQueueSize),
		lastReportTime: make(map[*Span]time.Time),
		traversePeriod: defaultTraversePeriod,
	}
	go p.traverse()
	return p
}

func (p *traversalReportPolicy) add(span *Span) {
	p.lock.Lock()
	defer p.lock.Unlock()
	if _, ok := p.lastReportTime[span]; !ok {
		p.lastReportTime[span] = time.Now()
	}
}

func (p *traversalReportPolicy) remove(span *Span) {
	p.lock.Lock()
	defer p.lock.Unlock()
	if _, ok := p.lastReportTime[span]; ok {
		delete(p.lastReportTime, span)
	}
}

func (p *traversalReportPolicy) reportQueue() <-chan []*Span {
	return p.queue
}

func (p *traversalReportPolicy) traverse() {
	for now := range time.Tick(p.traversePeriod) {
		...
		for span, lastReport := range p.lastReportTime {
			...
			} else if span.context.IsSampled() && now.After(lastReport.Add(span.flushInterval)) {
				// Only update span duration.
				span.duration = span.tracer.timeNow().Sub(span.startTime)
				toReport = append(toReport, span)
...
type PeriodicFlushReporter struct {
	r      Reporter
	lock   sync.Mutex
	policy reportPolicy
}

// NewPeriodicFlushReporter creates a new reporter that sends spans out of process even they
// are unfinished. It's a decorator of remoteReporter.
func NewPeriodicFlushReporter(policy reportPolicy, baseReporter Reporter) Reporter {
	reporter := &PeriodicFlushReporter{
		r:      baseReporter,
		policy: policy,
	}
	go func() {
		for spans := range reporter.policy.reportQueue() {
			for _, span := range spans {
				if span.isPeriodicReportEnabled() {
					reporter.r.Report(span)
				}
			}
		}
	}()
...
// Report implements Report() method of Reporter.
func (r *PeriodicFlushReporter) Report(span *Span) {
	r.r.Report(span)
	if span.isPeriodicReportEnabled() {
		r.policy.add(span)
	} else {
		r.policy.remove(span)
	}
}

// Close implements Close() method of Reporter by waiting for the queue to be drained.
func (r *PeriodicFlushReporter) Close() {
	r.r.Close()
}
