...
func (mounter *Mounter) doMount(mounterPath string, mountCmd string, source string, target string, fstype string, options []string, sensitiveOptions []string) error {
	...
	if err != nil {
		...
		if mounter.withSystemdMount && mounterPath == "" {
			// clean failed units (generated by "systemd-mount"), otherwise mount operation
			// will failed due to "mount units already exists" even when target recovers
			// todo, "systemd-run" may also have this problem, but its mount units are always generated randomly
			klog.V(2).Infof("Try to reset failed mount units: %s", string(output))
			// the generated mount unit name contains char encode in hex, eg \x2d for -, \x7e for ~
			re := regexp.MustCompile(`^Failed to start transient mount unit: Unit ([a-z0-9\.\-\x00-\x7F]+.mount) already exists\.$`)
			mg := re.FindStringSubmatch(strings.TrimSpace(string(output)))
			if len(mg) == 2 {
				klog.V(2).Infof("Extracted transient mount unit: %s", mg[1])
				cmd := exec.Command("systemctl", "reset-failed", mg[1])
				if err := cmd.Run(); err != nil {
					klog.V(2).Infof("reset failed units %s failed: %s", mg[1], err)
				}
			}
		}
...
func detectSystemd() (bool, bool) {
	...
	if err != nil {
		...
		if len(mg) == 2 {
			klog.V(2).Infof("Extracted transient scope unit: %s", mg[1])
