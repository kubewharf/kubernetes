...
type watchChan struct {
	...
	incomingEventChan chan *api.Event
	resultChan        chan watch.Event
	errChan           chan error
	ignoreModPrev     bool
}

func newWatcher(client client.Client, codec runtime.Codec, versioner storage.Versioner, transformer value.Transformer) *watcher {
	return &watcher{
		client:      client,
		codec:       codec,
		versioner:   versioner,
...
func (w *watcher) createWatchChan(ctx context.Context, key string, rev int64, recursive bool, pred storage.SelectionPredicate) *watchChan {
	wc := &watchChan{
		...
		recursive:         recursive,
		internalPred:      pred,
		incomingEventChan: make(chan *api.Event, incomingBufSize),
...
func (wc *watchChan) sync() error {
	revision, kvs, _, _, err := wc.watcher.client.Range(wc.ctx, wc.key, common.GetPrefixRangeEnd(wc.key), 0, 0)
	if err != nil {
		return err
	}
	wc.initialRev = revision
	for _, kv := range kvs {
		wc.sendEvent(api.ParseKV(kv))
	}
	return nil
}
...
func (wc *watchChan) startWatching(watchClosedCh chan struct{}) {
	...
	wch := wc.watcher.client.Watch(wc.ctx, wc.key, wc.initialRev+1, wc.recursive)
	for wres := range wch {
		if wres.Err != nil {
			err := wres.Err
		...
		for _, e := range wres.Events {
			err := api.ValidateEvent(e, wc.ignoreModPrev)
			if err != nil {
				logWatchChannelErr(err)
				wc.sendError(err)
				return
...
func (wc *watchChan) transform(e *api.Event) (res *watch.Event) {
...
func (wc *watchChan) sendEvent(e *api.Event) {
...
func (wc *watchChan) prepareObjs(e *api.Event) (curObj runtime.Object, oldObj runtime.Object, err error) {
	if !e.IsDeleted {
