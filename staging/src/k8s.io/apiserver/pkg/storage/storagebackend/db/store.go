...
func New(c client.Client, codec runtime.Codec, prefix string, transformer value.Transformer, pagingEnabled bool, maxLimit int) storage.Interface {
	return newStore(c, pagingEnabled, codec, prefix, transformer, maxLimit)
}

func newStore(c client.Client, pagingEnabled bool, codec runtime.Codec, prefix string, transformer value.Transformer, maxLimit int) *store {
	versioner := common.APIObjectVersioner{}
	result := &store{
		client:        c,
		codec:         codec,
		versioner:     versioner,
...
func (s *store) Get(ctx context.Context, key string, resourceVersion string, out runtime.Object, ignoreNotFound bool) error {
	...
	startTime := time.Now()
	dbRevision, kvRevision, value, err := s.client.Get(ctx, key)
	metrics.RecordEtcdRequestLatency("get", getTypeName(out), startTime)
	if err != nil {
		return err
	}
	if err = s.ensureMinimumResourceVersion(resourceVersion, uint64(dbRevision)); err != nil {
		return err
	}
	...
	data, _, err := s.transformer.TransformFromStorage(value, authenticatedDataString(key))
	if err != nil {
		return storage.NewInternalError(err.Error())
	}

	return decode(s.codec, s.versioner, data, out, kvRevision)
}
...
func (s *store) Create(ctx context.Context, key string, obj, out runtime.Object, ttl uint64) error {
	...
	startTime := time.Now()
	success, revision, err := s.client.Create(ctx, key, newData, ttl)
	...
	if out != nil {
		return decode(s.codec, s.versioner, data, out, revision)
	}
	return nil
}
...
func (s *store) conditionalDelete(ctx context.Context, key string, out runtime.Object, v reflect.Value, preconditions *storage.Preconditions, validateDeletion storage.ValidateObjectFunc) error {
	startTime := time.Now()
	_, revision, value, err := s.client.Get(ctx, key)
	metrics.RecordEtcdRequestLatency("get", getTypeName(out), startTime)
	if err != nil {
		return err
	}
	for {
		origState, err := s.getState(key, value, revision, v, false)
		if err != nil {
			return err
		}
		...
		startTime := time.Now()
		var success bool
		success, revision, value, err = s.client.Delete(ctx, key, revision)
...
func (s *store) GuaranteedUpdate(
	...
	trace := utiltrace.New("GuaranteedUpdate kubebain", utiltrace.Field{"type", getTypeName(out)})
	...
	getCurrentState := func() (*objState, error) {
		...
		return s.getState(key, value, revision, v, ignoreNotFound)
	}
	...
	for {
		...
		startTime := time.Now()
		success, revision, value, err := s.client.Update(ctx, key, origState.rev, newData, ttl)
		...
		if !success {
			...
			origState, err = s.getState(key, value, revision, v, ignoreNotFound)
			if err != nil {
				return err
			}
...
func (s *store) GetToList(ctx context.Context, key string, resourceVersion string, pred storage.SelectionPredicate, listObj runtime.Object) error {
	trace := utiltrace.New("GetToList kubebrain",
	...
	key = path.Join(s.pathPrefix, key)
	startTime := time.Now()
	dbRevision, revision, value, err := s.client.Get(ctx, key)
	metrics.RecordEtcdRequestLatency("get", getTypeName(listPtr), startTime)
	if err != nil {
		return err
	}
	if err = s.ensureMinimumResourceVersion(resourceVersion, uint64(revision)); err != nil {
		return err
	}
	...
	if len(value) > 0 {
		...
		if err := appendListItem(v, data, uint64(revision), pred, s.codec, s.versioner, newItemFunc); err != nil {
			return err
		}
	}
	// update version with cluster level revision
	return s.versioner.UpdateList(listObj, uint64(dbRevision), "", nil)
}
...
func (s *store) Count(key string) (int64, error) {
	...
	startTime := time.Now()
	count, err := s.client.Count(context.Background(), key, common.GetPrefixRangeEnd(key))
...
func (s *store) List(ctx context.Context, key, resourceVersion string, pred storage.SelectionPredicate, listObj runtime.Object) error {
	trace := utiltrace.New("List kubebrain",
		utiltrace.Field{"key", key},
		utiltrace.Field{"resourceVersion", resourceVersion},
		utiltrace.Field{"limit", pred.Limit},
		utiltrace.Field{"continue", pred.Continue})
	...
	case s.pagingEnabled && len(pred.Continue) > 0:
		...
		end = common.GetPrefixRangeEnd(keyPrefix)
		key = continueKey
	...
	var lastKey []byte
	var hasMore bool
	var rangeRevision int64
	var kvs []*api.KeyValue
	var count int64
	for {
		startTime := time.Now()
		rangeRevision, kvs, hasMore, count, err = s.client.Range(ctx, key, end, pred.Limit, withRev)
		metrics.RecordEtcdRequestLatency("list", getTypeName(listPtr), startTime)
		if err != nil {
			return interpretListError(err, len(pred.Continue) > 0, continueKey, keyPrefix)
		}
		if err = s.ensureMinimumResourceVersion(resourceVersion, uint64(rangeRevision)); err != nil {
			return err
		}
		...
		for _, kv := range kvs {
			...
			if err := appendListItem(v, data, uint64(kv.Rev), pred, s.codec, s.versioner, newItemFunc); err != nil {
				return err
			}
		}
	...
	if hasMore {
		...
		// bugfix continue key compare result not right
		next, err := encodeContinue(string(lastKey)+",", keyPrefix, returnedRV)
		if err != nil {
			return err
		}
		...
		if utilfeature.DefaultFeatureGate.Enabled(features.RemainingItemCount) {
			if pred.Empty() {
				c := int64(count) - int64(pred.Limit)
...
func (s *store) getState(key string, value []byte, revision int64, v reflect.Value, ignoreNotFound bool) (*objState, error) {
	state := &objState{
		meta: &storage.ResponseMeta{},
	}
	...
	if len(value) == 0 {
		...
		data, stale, err := s.transformer.TransformFromStorage(value, authenticatedDataString(key))
		if err != nil {
