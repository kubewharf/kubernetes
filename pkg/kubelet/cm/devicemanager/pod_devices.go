...
func (pdev *podDevices) allAllocatedDevicesHealthy(resourceName string, healthyDevices sets.String) sets.String {
	pdev.RLock()
	defer pdev.RUnlock()

	reAllocatePods := sets.NewString()

	for podUID, containers := range pdev.devs {
		for _, resources := range containers {
			for resource, devices := range resources {
				if resource == resourceName {
					if !healthyDevices.IsSuperset(devices.deviceIds.Devices()) {
						klog.Warningf("resource: %s devices: %+v for podUID: %v isn't all healthy, needs re-allocate device plugin resources", resourceName, devices.deviceIds.Devices(), podUID)
						reAllocatePods.Insert(podUID)
					} else {
						break
					}
				}
