...
func (m *ManagerImpl) readCheckpoint() error {
	...
	if err != nil {
		if err == errors.ErrCheckpointNotFound {
			...
			klog.Infof("Failed to read data from checkpoint: %s, err: %s", kubeletDeviceManagerCheckpoint, err)
			return nil
		}
		...
		if errv1 != nil {
			klog.Infof("Failed to read checkpoint V1 file, err: %v", errv1)
		...
		klog.Infof("Read data from a V1 checkpoint: %s", kubeletDeviceManagerCheckpoint)
	}
...
func (m *ManagerImpl) writeRefinedResourceToStateFile(devices []pluginapi.Device, path string) error {
	content, err := json.Marshal(devices)
	if err != nil {
		return fmt.Errorf("Fail to marshal devices: %v", err)
	}
	m.mutex.Lock()
	defer m.mutex.Unlock()
	file, err := os.OpenFile(path, os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0644)
	if err != nil {
		return fmt.Errorf("Fail to open refined resource state file: %v", err)
	}
	defer file.Close()
	if _, err = file.Write(content); err != nil {
		return fmt.Errorf("Fail to write refined resource state file: %v", err)
	}
	return nil
}
...
func getRefinedResourceFromDevices(devices []pluginapi.Device) (map[string]string, map[string]string, map[string]string, map[string]string) {
	...
	for _, dev := range devices {
		...
		case numericResourceType:
			refinedNumericResources[dev.GetID()] = dev.GetProperties()["size"]
