...
func isSkippedPod(pod *v1.Pod, isFirstAdmit, hitSkipAnnoKey bool) bool {
...
func PodHasAnyTargetAnnoKey(pod *v1.Pod, targetAnnoKeys []string) bool {
	if pod == nil {
		return false
	}

	for _, targetAnnoKey := range targetAnnoKeys {
		if _, found := pod.Annotations[targetAnnoKey]; found {
			return true
		}
	}
