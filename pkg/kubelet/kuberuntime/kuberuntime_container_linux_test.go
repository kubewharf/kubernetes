...
func makeExpectedConfig(m *kubeGenericRuntimeManager, pod *v1.Pod, containerIndex int) *runtimeapi.ContainerConfig {
	...
	expectedConfig := &runtimeapi.ContainerConfig{
		...
		Tty:         container.TTY,
		Linux:       m.generateLinuxContainerConfig(container, pod, new(int64), "", nil, nil, newContainerAnnotations(container, pod, restartCount, opts)),
	}
...
func TestGenerateLinuxContainerConfigNamespaces(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := m.generateLinuxContainerConfig(&tc.pod.Spec.Containers[0], tc.pod, nil, "", tc.target, nil, nil)
