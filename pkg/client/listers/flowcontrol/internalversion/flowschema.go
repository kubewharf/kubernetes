...
type FlowSchemaLister interface {
	...
	List(selector labels.Selector) (ret []*flowcontrol.FlowSchema, err error)
	// Get retrieves the FlowSchema from the index for a given name.
	Get(name string) (*flowcontrol.FlowSchema, error)
...
// List lists all FlowSchemas in the indexer.
func (s *flowSchemaLister) List(selector labels.Selector) (ret []*flowcontrol.FlowSchema, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*flowcontrol.FlowSchema))
	})
	return ret, err
}

// Get retrieves the FlowSchema from the index for a given name.
func (s *flowSchemaLister) Get(name string) (*flowcontrol.FlowSchema, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(flowcontrol.Resource("flowschema"), name)
	}
	return obj.(*flowcontrol.FlowSchema), nil
}
