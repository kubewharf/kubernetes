...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.Int32Var(&c.RegistryMaxConcurrency, "registry-max-concurrency", c.RegistryMaxConcurrency, "Maximum concurrency of images to pull. If 0, unlimited.")
	...
	fs.StringVar(&c.TopologyManagerPolicy, "topology-manager-policy", c.TopologyManagerPolicy, "Topology Manager policy to use. Possible values: 'none', 'best-effort', 'restricted', 'single-numa-node', 'socket-strict'.")
	...
	fs.Int32Var(&c.EvictionHardMaxPodGracePeriod, "eviction-hard-max-pod-grace-period", c.EvictionHardMaxPodGracePeriod, "Maximum allowed grace period (in seconds) to use when terminating pods in response to a hard eviction threshold being met.  If negative, defer to pod specified value.")
	...
	fs.Var(&c.GuaranteedQOSHostPortRange, "host-port-high-priority-range", "Dynamic host port range, reserved for high priority services, example: '30000-32767', inclusive for both start and ending ports")
	fs.Var(&c.HostPortRange, "host-port-range", "A port range to reserve for dynamic host port.  Example: '30000-32767'.  Inclusive at both ends of the range.")
}
